...
function renderQ(){
  ...
  $('sePrev').disabled=SE.idx===0;
  const isLast=SE.idx===SE.q.items.length-1;
  const seNext=$('seNext');
  seNext.textContent=isLast?'✅ Submit attempt':'Next →';
  seNext.className=isLast?'btn btn-ai':'btn btn-p';
}
document.addEventListener('DOMContentLoaded',()=>{
  const sePrev=$('sePrev'), seNext=$('seNext'), secureEnv=$('secureEnv');
  if(sePrev) sePrev.onclick=()=>{if(SE.idx>0){SE.idx--;renderQ();}};
  if(seNext) seNext.onclick=()=>{
    if(SE.idx<SE.q.items.length-1){SE.idx++;renderQ();return;}
...
function endQuiz(reason){
  ...
  setTimeout(()=>{
    toast(LAST_RESULT.practice?'Practice finished — recorded to your personal growth log only.':'Score auto-recorded to the class scoring sheet ⚡');
    showQuizResults();
  },200);
}
...
function recordGradedAttempt(qid,q,ans,score,n,weak){
  ...
  else s.records.push({comp:'Quiz', label:q.title, score, max:n, auto:true, reviewId:qid, presc});
...
function showQuizResults(){
  ...
  openSheet('🏁', (practice?'Practice Results — ':'Quiz Results — ')+q.title, reason+(practice?' · NOT graded — personal growth log only':' · score auto-recorded to the scoring sheet ⚡'), `
   <div class="res-score">
   ...
   <div class="grid g3" style="margin-bottom:18px">
     <div class="card"><h3>Performance summary — this attempt</h3>
     ...
     <div class="card"><h3>Score-based projection — running average <span class="ai-badge">PROJECTION</span></h3>
       <p style="font-size:12.5px;line-height:1.65">${practice
        ?'Practice attempts never change your scores. Sustained practice at this level projects a <b>+'+Math.max(0,pct-60)+'% mastery gain</b> on these topics in your personal growth log.'
        :'This score is recorded automatically into the EduPulse scoring sheet. Holding this level projects your <b>running average ≈ '+pct+'%</b> for this subject; one more low quiz on the same topics would pull the projection below the 75% mark. <span style="color:var(--mut)">Projection is score-based only — EduPulse does not compute term grades.</span>'}</p>
     </div>
...
function practiceModal(qid){
  ...
  <div class="frm">
   <div class="note-ai note" style="margin:0">
    ...
    • Your result is <b>never recorded to the class scoring sheet</b> and your instructor's records are untouched.<br>
